...
var config = function() {
  var options = {
    viewportWidth: 100,
    viewportHeight: 130,
    customEase: CustomEase.create("custom", "M0,0 C0.8,0 0.2,1 1,1")
  };
...
var helper = (function() {
  ...
  var getComputedTranslate = function(obj, i) {
    ...
    return mat ? parseFloat(mat[1].split(', ')[i]) : 0;
  }
  ...
  return {
    ...
    getComputedTranslate: getComputedTranslate
  };

})();

var viewportControl = (function () {
  var c = new config();
  var viewportWidth, viewportHeight;
  var viewportStatus = 0; // Default to Closed

  // Select Curtain and fade out when window loads
  var curtain = document.getElementById("curtain");
  helper.addClass(curtain, "hide");

  // Select Brandmark Assets for animation
  ////var viewportObj = document.getElementById("viewport");
  var wordmarkObj = document.getElementById("wordmark");
  var wordmarkFill = document.getElementById("wordmark-fill");
  var hitarea = document.getElementById("window-hitarea");
  var world = document.getElementById("world");
  var info = document.getElementById("info-card");
  // Select Guidelines for Animation
  var vLineA = document.getElementById("svg-line-vertical-a");
  var vLineB = document.getElementById("svg-line-vertical-b");
  var hLineA = document.getElementById("svg-line-horizontal-a");
  var hLineB = document.getElementById("svg-line-horizontal-b");


  // Select Viewport Controls
  bg = document.getElementById("background");
  var closeBtn = document.getElementById("close");
  // Listen for Selection of Window Contents to Open
  //bg.addEventListener("mousedown", openViewport, false);
  //bg.addEventListener("touchstart", openViewport, false);

  var _getViewportVariables = function() {
    // Get Window Size
    var windowW = window.innerWidth;
    var windowH = window.innerHeight;

    // Calculate Brandmark Position
    var markPosX = (windowW / 2) - (viewportWidth / 2);
    var markPosY = (windowH / 2) - ((viewportHeight + parseInt(wordmarkFill.getAttribute("height"))) / 2);
    var barPosY = markPosY + viewportHeight;
    var barPosX = markPosX + viewportWidth;
    // Calculate Screen Aspect Ratio Modified Mark Size
    if (windowW >= windowH) {
      var ratioY = 110; // Multiplier
      var ratioX = Math.round((windowW / windowH) * ratioY);
    } else {
      var ratioX = 130; // Multiplier
      var ratioY = Math.round((windowH / windowW) * ratioX);
    }
    // Calculate Modified Mark Line Position
    var lineXa = (windowW / 2) - (ratioX / 2);
    var lineYa = (windowH / 2) - (ratioY / 2);
    var lineXb = (windowW / 2) + (ratioX / 2);
    var lineYb = (windowH / 2) + (ratioY / 2);
  ...
  // Reset Animated Guidelines to 0
  var _resetGuidelines = function(x, y, w, h) {
    /* Vertical Line 1 */
    helper.setAttributes(vLineA, { "x1": x, "y1": 0, "x2": x, "y2": 0 });
    /* Vertical Line 2 */
    helper.setAttributes(vLineB, { "x1": x + viewportWidth, "y1": h, "x2": x + viewportWidth, "y2": h });
    /* Horizontal Line 1 */
    helper.setAttributes(hLineA, { "x1": w, "y1": y, "x2": w, "y2": y });
    /* Horizontal Line 2 */
    helper.setAttributes(hLineB, { "x1": 0, "y1": y + viewportHeight, "x2": 0, "y2": y + viewportHeight });
  }

  // Resize Viewport
  var setViewportSize = function(w, h)  {
    if (typeof(w)==="NaN" || typeof(w)==="undefined") { w = 100; }
    if (typeof(h)==="NaN" || typeof(h)==="undefined") { h = 130; }
    viewportWidth = parseInt(w);
    viewportHeight = parseInt(h);

    var viewVars = new _getViewportVariables();
    var hitareaString = "display: block; width: " + viewportWidth + "px; height: " + viewportHeight + "px; margin-top: -" + ((viewportHeight / 2) + 15) + "px; margin-left: -" + viewportWidth / 2 + "px";

    switch(viewportStatus) {
      case 0: // Closed
      // Reset Animated Guidelines to 0
      _resetGuidelines(viewVars.markPosX, viewVars.markPosY, viewVars.windowW, viewVars.windowW);
      // Match Wordmark to Brandmark width
      helper.setAttributes(wordmarkFill, {"width": viewportWidth});
      hitarea.style.cssText = hitareaString;

      // Position Brand Mark in Center
      bg.style.clip = "rect(" + viewVars.markPosY + ", " + viewVars.barPosX + ", " + viewVars.barPosY + ", " + viewVars.markPosX + ")";
      // Position Work Mark below Viewport
      helper.setAttributes(wordmarkObj, {"webkitTransform": "translate(" + viewVars.markPosX + ", " + viewVars.barPosY + ")", "transform": "translate(" + viewVars.markPosX + ", " + viewVars.barPosY + ")"});
      break;
      case 1: // Transition
      // DO NOthing
      break;
      ...
      // Reset Animated Guidelines to 0
      _resetGuidelines(viewVars.markPosX, viewVars.markPosY, viewVars.windowW, viewVars.windowW);
      // Position Brand Mark to window Bounds
      bg.style.clip = "rect(" + 0 + ", " + viewVars.windowW + ", " + viewVars.windowH + ", " + 0 + ")";
      // Expand Wordmark to window width
      helper.setAttributes(wordmarkFill, {"width": viewVars.windowW});
      // Position Work Mark to window Bottom
      helper.setAttributes(wordmarkObj, {"webkitTransform": "translate(" + 0 + ", " + viewVars.windowH + ")", "transform": "translate(" + 0 + ", " + viewVars.windowH + ")"});
      break;
    }
  }

  var openViewport = function() {
    // Set Resize Status and remove Listeners
    viewportStatus = 1;
    bg.removeEventListener("mousedown", openViewport, false);

    hitarea.style.display = "none";
    helper.addClass(info, "inactive");

    var viewVars = new _getViewportVariables();

    // Create Animation Timeline
    var tl = new TimelineLite({onComplete:_viewportIsOpen});
    // Draw in Guidelines
    tl.to(vLineA, 1.2, {attr:{y2: viewVars.windowH}, ease:c.options.customEase}, 0)
    .to(vLineB, 1.2, {attr:{y2: 0}, ease:c.options.customEase}, 0)
    .to(hLineA, 1, {attr:{x2: 0}, ease:c.options.customEase}, 0.3)
    .to(hLineB, 1, {attr:{x2: viewVars.windowW}, ease:c.options.customEase}, 0.3)
    // Animate Guidelines and Viewport to Screen Aspect Radio
    .to(vLineA, 0.6, {attr:{x1: viewVars.lineXa, y1: 0, x2: viewVars.lineXa, y2: viewVars.windowH}, ease:c.options.customEase}, 1.4)
    .to(vLineB, 0.6, {attr:{x1: viewVars.lineXb, y1: viewVars.windowH, x2: viewVars.lineXb, y2: 0}, ease:c.options.customEase}, 1.4)
    .to(hLineA, 0.6, {attr:{x1: viewVars.windowW, y1: viewVars.lineYa, x2: 0, y2: viewVars.lineYa}, ease:c.options.customEase}, 1.4)
    .to(hLineB, 0.6, {attr:{x1: 0, y1: viewVars.lineYb, x2: viewVars.windowW, y2: viewVars.lineYb}, ease:c.options.customEase}, 1.4)
    .call(helper.addClass, [bg, "half-transition"], null, 1.3)
    .set(bg, {"clip": "rect(" + viewVars.lineYa + ", " + viewVars.lineXb + ", " + viewVars.lineYb + ", " + viewVars.lineXa+ ")"}, 1.4)
    .to(wordmarkObj, 0.6, {attr:{"webkitTransform": "translate(" + viewVars.lineXa + ", "+ viewVars.lineYb + ")", "transform": "translate(" + viewVars.lineXa + ", "+ viewVars.lineYb + ")"}, ease:c.options.customEase}, 1.4)
    .to(wordmarkFill, 0.6, {attr:{width: viewVars.ratioX}, ease:c.options.customEase}, 1.4)
    // Animate Guidelines and Viewport to Window Bounds
    .to(vLineA, 1, {attr:{x1: 0, y1: 0, x2: 0, y2: viewVars.windowH}, ease: c.options.customEase}, 2.1)
    .to(vLineB, 1, {attr:{x1: viewVars.windowW, y1: viewVars.windowH, x2: viewVars.windowW, y2: 0}, ease: c.options.customEase}, 2.1)
    .to(hLineA, 1, {attr:{x1: viewVars.windowW, y1: 0, x2: 0, y2: 0}, ease: c.options.customEase}, 2.1)
    .to(hLineB, 1, {attr:{x1: 0, y1: viewVars.windowH, x2: viewVars.windowW, y2: viewVars.windowH}, ease: c.options.customEase}, 2.1)
    .call(helper.removeClass, [bg, "half-transition"], null, 2.0)
    .call(helper.addClass, [bg, "full-transition"], null, 2.0)
    .set(bg, {"clip": "rect(" + 0 + ", " + viewVars.windowW + ", " + viewVars.windowH + ", " + 0 + ")"}, 2.1)
    .to(wordmarkObj, 1, {attr:{"webkitTransform": "translate(" + 0 + ", "+ viewVars.windowH + ")", "transform": "translate(" + 0 + ", "+ viewVars.windowH + ")"}, ease:c.options.customEase}, 2.1)
    .to(wordmarkFill, 1, {attr:{width: viewVars.windowW}, ease:c.options.customEase}, 2.1)
    // Open Navigation
    .to("#nav", 0.8, {transform: "translateX(-0px)", ease:c.options.customEase}, 3)
    .call(helper.removeClass, [bg, "full-transition"], null, 3.0);
  }

  // Set Open viewport status and listeners
  var _viewportIsOpen = function() {
    viewportStatus = 2;
    resize();
    viewportLens.init();
    helper.addClass(world, "active");
    world.style.pointerEvents = "all";
  }
  ...
  var closeViewport = function()  {
    ...
    // Create Animation Timeline
    var tl = new TimelineLite({onComplete:_viewportIsClosed});
    // Animate Navigation Out
    tl.to("#nav", 0.6, {transform: "translateX(80px)", ease:c.options.customEase}, 0)
    // Animate Viewport Closed
    .call(helper.addClass, [bg, "full-transition"], null, 0.6)
    .set(bg, {"clip": "rect(" + viewVars.markPosY + ", " + viewVars.barPosX + ", " + viewVars.barPosY + ", " + viewVars.markPosX + ")"}, 0.7)
    .to(wordmarkObj, 1, {attr:{"webkitTransform": "translate(" + viewVars.markPosX + ", "+ viewVars.barPosY + ")", "transform": "translate(" + viewVars.markPosX + ", "+ viewVars.barPosY + ")"}, ease:c.options.customEase}, 0.7)
    .to(wordmarkFill, 1, {attr:{width: viewportWidth}, ease:c.options.customEase}, 0.7)
    .call(helper.removeClass, [bg, "full-transition"], null, 1.7);
  }

  // Set Closed viewport status and listeners
  var _viewportIsClosed = function() {
    viewportStatus = 0;
    resize();
    viewportLens.init();
    world.style.pointerEvents = "none";
    helper.removeClass(info, "inactive");
    helper.removeClass(world, "active");
  }
...
var viewportLens = (function () {
  ////var viewportObj = document.getElementById("viewport-lens");
  var wordmarkObj = document.getElementById("brandmark-lens");
  var backgroundDup = document.getElementById("background-scale");
  var hitarea = document.getElementById("hitarea");
  var hitareaPartial = document.getElementById("hitarea-partial");
  var lens = {};
  var paraScale = document.getElementById("parallax-scale");

  var c = new config(); // Get Easing Settings
  // Set Default Position
  ////TweenMax.to(viewportObj, 0, {transform: "matrix(1,0,0,1,130,0)"});
  //TweenMax.to(wordmarkObj, 0, {transform: "matrix(1,0,0,1,130,0)"});
  backgroundDup.style.clip = "rect(" + -130 + ", " + 230 + ", " + 0 + ", " + 130 + ")";

  var _revealLens = function() {
    hitarea.removeEventListener("mouseover", _revealLens, false);
    hitareaPartial.removeEventListener("mouseover", _peekLens, false);
    hitareaPartial.removeEventListener("mouseout", _hideLens, false);

    TweenMax.set(wordmarkObj, {"transform": "translate(0, 250px)", "transition-duration": "1s"});
    TweenMax.set(backgroundDup, {"transition-duration": "1s"});
    TweenMax.set(backgroundDup, {"clip": "rect(" + 120 + ", " + 230 + ", " + 250 + ", " + 130 + ")"});

    lens = Draggable.create(wordmarkObj, {
      onDrag:function(e) {
          _update(e);
        },
      onDragEnd:function(e) {
       //checks if at least 20 pixels are overlapping:
       if (this.hitTest(hitarea, 20)) {
         TweenMax.set(backgroundDup, {"transition-duration": "0.5s"});
         TweenMax.set(backgroundDup, {"clip": "rect(" + -130 + ", " + 230 + ", " + 0 + ", " + 130 + ")"});
         TweenMax.set(wordmarkObj, {"transform": "translate(0, 0)", "transition-duration": "0.5s"});
         setTimeout(function() {
           init();
         }, 500);
       }
  ...
  var _peekLens = function(e) {
    hitareaPartial.removeEventListener("mouseover", _peekLens, false);
    TweenMax.set(wordmarkObj, {"transform": "translate(0, 20px)", "transition-duration": "0.4s"});
    TweenMax.set(backgroundDup, {"clip": "rect(" + -110 + ", " + 230 + ", " + 20 + ", " + 130 + ")"});
    hitareaPartial.addEventListener("mouseout", _hideLens, false);
  }

  var _hideLens = function() {
    hitareaPartial.removeEventListener("mouseout", _hideLens, false);
    TweenMax.set(backgroundDup, {"transition-duration": "0.4s"});
    TweenMax.set(backgroundDup, {"clip": "rect(" + -130 + ", " + 230 + ", " + 0 + ", " + 130 + ")"});
    TweenMax.set(wordmarkObj, {"transform": "translate(0, 0)", "transition-duration": "0.4s"});
    hitareaPartial.addEventListener("mouseover", _peekLens, false);
  }

  var _update = function(e) {
    TweenMax.set(backgroundDup, {"transition-duration": "0s"});
    TweenMax.set(wordmarkObj, {"transition-duration": "0s"});
    // Get the Wordmark Transform Matrix
    var transform = wordmarkObj.getAttribute("transform");
    // Apply wordmark transform matrix to the viewport
    var wordmarkX = helper.getComputedTranslate(wordmarkObj, 4);
    var wordmarkY = helper.getComputedTranslate(wordmarkObj, 5);

    // Calculate Brandmark Position
    var markPosX = wordmarkX + 130;
    var markPosY = wordmarkY - 130;
    var barPosY = markPosY + 130;
    var barPosX = markPosX + 100;

    TweenMax.set(backgroundDup, {"clip": "rect(" + markPosY + ", " + barPosX + ", " + barPosY + ", " + markPosX + ")"});

    // Set scale origin for magnified layer
    paraScale.style.webkitTransformOrigin = (e.pageX) + " " + (e.pageY);
    paraScale.style.transformOrigin = (e.pageX) + " " + (e.pageY);
  }

  var init = function() {
    // Kill old draggable instance
    TweenMax.set(wordmarkObj, {clearProps:"all"});
    // Reset positions
    TweenMax.set(backgroundDup, {"transition-duration": "0.4s"});
    TweenMax.set(backgroundDup, {"clip": "rect(" + -130 + ", " + 230 + ", " + 0 + ", " + 130 + ")"});

    if (lens[0] && lens[0]._eventTarget.onselectstart != null) {
      lens[0].kill();
    }
    // Add rollover hit areas
    hitarea.addEventListener("mouseover", _revealLens, false);
    hitareaPartial.addEventListener("mouseover", _peekLens, false);
  }
...
var resize = function() {
  var c = new config();
  viewportControl.setViewportSize(c.options.viewportWidth, c.options.viewportHeight);
};
...
var parallax = (function(e) {
  ...
  var layerSpeed = [1000, 500, 200, 24, 20, 3]

  // Scene 01 Parallax
  var scene01 = function(e) {

    for (var i = 0; i < layers.length; i++) {
      //maxMove[layer] = layer.offsetWidth / 30;
      centerX = layers[i].offsetLeft + (layers[i].offsetWidth / 2);
      centerY = layers[i].offsetTop + (layers[i].offsetHeight / 2);
      let distX = e.pageX - centerX,
          distY = e.pageY - centerY;
          //console.log(j);
      layers[i].style.webkitTransform =  "translate("+(-1 * distX) / layerSpeed[i] + "px," + (-1 * distY) / layerSpeed[i] + "px)";
      layers[i].style.transform =  "translate("+(-1 * distX) / layerSpeed[i] + "px," + (-1 * distY) / layerSpeed[i] + "px)";
...
var reticleBehavior = (function(){
    var reticleCursor = document.getElementById('reticle');
    var downTimer;

    var _moveReticle = function(e) {
      var x = e.pageX;
      var y = e.pageY;
      reticleCursor.style.left = x + "px";
      reticleCursor.style.top = y + "px";
    }

    var _forwardFuse = function(e) {
      helper.addClass(reticleCursor, 'active');
      downTimer = setTimeout(function() {
        _select(e);
      }, 500);
    }

    var _backwardFuse = function(e) {
      helper.removeClass(reticleCursor, 'active');
      clearTimeout(downTimer);
    }

    var _reset = function() {
      helper.removeClass(reticleCursor, 'transition');
      helper.removeClass(reticleCursor, 'active');
      setTimeout(function() {
        helper.removeClass(reticleCursor, 'active');
        helper.addClass(reticleCursor, 'transition');
      }, 10);
      clearTimeout(downTimer);
    }

    var _success = function() {
      helper.addClass(reticleCursor, 'complete');
      helper.removeClass(reticleCursor, 'transition');
      window.removeEventListener("mousedown", _forwardFuse, false);
      setTimeout(function() {
        helper.removeClass(reticleCursor, 'active');
        helper.removeClass(reticleCursor, 'complete');
        helper.addClass(reticleCursor, 'hidden');
      }, 1000);
      setTimeout(function() {
        helper.addClass(reticleCursor, 'transition');
        helper.removeClass(reticleCursor, 'hidden');
        window.addEventListener("mousedown", _forwardFuse, false);
      }, 2500);
      clearTimeout(downTimer);
    }

    var _select = function(e) {
      if (e.target.id == "window-hitarea") {
        viewportControl.openViewport();
        _success();
      } else if (e.target.id == "close") {
        viewportControl.closeViewport();
        _success();
      } else {
        _reset();
      }
    ...
    window.addEventListener("mousemove", _moveReticle, false);
    window.addEventListener("mousedown", _forwardFuse, false);
    window.addEventListener("mouseup", _backwardFuse, false);

    return {
      _forwardFuse: _forwardFuse
    };
...
var createStars = function(a, b, c, d) {
  for (var i = 0; i < b; i++) {
      var x = Math.random() * c.offsetWidth;
      var y = Math.random() * c.offsetHeight;
      var radius = Math.random() * 0.9;
      var hue = d[getRandom(0,d.length - 1)];
      var sat = getRandom(50,100);
      var opac = getRandomOpacity(0, 0.5);
      a.beginPath();
      a.arc(x, y, radius, 0, 360);
      a.fillStyle = "hsla(" + hue + ", " + sat + "%, 88%, "+ opac +")";
      a.fill();
  }
};

var createNovas = function(a, b, c, d) {
  for (var i = 0; i < b; i++) {
      var x = Math.random() * c.offsetWidth;
      var y = Math.random() * c.offsetHeight;
      ...
      var hue = d[getRandom(0,d.length - 1)];
      var sat = getRandom(50,100);
      var opac = Math.random(0.3,0.7);
      a.beginPath();
      a.arc(x, y, radius, 0, 360);
      a.fillStyle = "hsla(" + hue + ", " + sat + "%, 82%, "+opac+")";
      a.shadowBlur = 50;
      a.shadowColor = "hsla(" + hue + ", " + sat + "%, 82%, "+opac+")";
      a.fill();
  }
};

var starfield = (function() {
    // Set Animation Frame Request
    var requestAnimationFrame = window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || window.msRequestAnimationFrame || function( callback ){
      window.setTimeout(callback, 1000 / 60);
    };
	  window.requestAnimationFrame = requestAnimationFrame;

    // Initialize Shooting Star Field
    var background = document.getElementById("shootingStar"),
    ...
    var _animate = function(){
      bgCtx.fillStyle = '#000';
      bgCtx.fillRect(0,0,width,height);
      bgCtx.fillStyle = '#FFFDC7';
      bgCtx.strokeStyle = '#FFFDC7';
...
(function (){
  ...
  // Listen for Selection of Window Contents to Open
  //bg.addEventListener("mousedown", viewportControl.openViewport, false);
  //bg.addEventListener("touchstart", viewportControl.openViewport, false);

  // Listen for Viewport Changes and adapted
  ...
  createStars(context, stars, canvas, colorrange);
  createNovas(context, novas, canvas, colorrange);
